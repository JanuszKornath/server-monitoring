...
for DISK in $DISKS; do
    ...
    # HTML Output
    echo "<div style='margin-bottom: 30px; border: 1px solid #ccc; padding: 15px; border-radius: 5px;'>" >> "$MAIL_BODY"
    echo "<h3 style='margin-top:0; color:#2980b9;'>Disk: $DEVICE</h3>" >> "$MAIL_BODY"
    echo "<p><b>Modell:</b> $MANUFACTURER $MODEL<br><b>Laufzeit:</b> $POWER_ON_HOURS Stunden</p>" >> "$MAIL_BODY"

    echo "<table border='1' cellspacing='0' cellpadding='4' style='border-collapse:collapse; width:100%; margin-bottom:15px;'>" >> "$MAIL_BODY"
    ...
    for ITEM in "Reallocated_Sectors:$REALLOC:50" "Pending_Sectors:$PENDING:0" "Offline_Uncorrectable:$OFFLINE:0" "UDMA_CRC_Errors:$CRC:100"; do
        ...
        COLOR="green"; MSG="OK"
        if [ "$VALUE" -gt "$THRESH" ]; then COLOR="red"; MSG="KRITISCH"
        elif [ "$VALUE" -gt 0 ]; then COLOR="orange"; MSG="WARNUNG"; fi
        echo "<tr><td>$NAME</td><td>$VALUE</td><td style='color:$COLOR; font-weight:bold;'>$MSG</td></tr>" >> "$MAIL_BODY"
    done
    echo "</table>" >> "$MAIL_BODY"

    # --- Historie-Tabelle ---
    echo "<p><b>Historie (Letzte 7 Läufe):</b></p>" >> "$MAIL_BODY"
    echo "<pre style='background:#f8f9fa; padding:10px; border-left:4px solid #3498db; font-family: monospace;'>" >> "$MAIL_BODY"
    echo "Datum      | Realloc | Pending | Offline" >> "$MAIL_BODY"
    echo "-----------|---------|---------|---------" >> "$MAIL_BODY"
    awk '{printf "%s | %7s | %7s | %7s\n", $1, $2, $3, $4}' "$HISTORY_FILE" >> "$MAIL_BODY"
    echo "</pre></div>" >> "$MAIL_BODY"
done
